  ...
  if conf.database == "cassandra" then
    ...
    for _, name in ipairs({
      ...
      elseif number_of_values == 2 then
        ...
        if not read_consistency or not write_consistency then
          errors[#errors + 1] = fmt("%s has an invalid value '%s': %s", name,
                                    concat(option, ", "),
                                    "when using two values, specify both " ..
                                    "read and write consistency, e.g. " ..
                                    "'read:ONE, write:ONE'")
        end
      ...
      if read_consistency and write_consistency then
        if opts.starting and is_dc_aware then
          local suggestion
